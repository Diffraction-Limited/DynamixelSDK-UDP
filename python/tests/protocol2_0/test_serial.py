...
steps = 5
while steps > 0:  # Adjust the range for the desired number of iterations
    random_position = random.randint(0, 4056)
    dxl_write_goal_position(DXL_ID, random_position)
    time.sleep(2)  # Add a delay to allow the Dynamixel to move to the position
    dxl_read_present_position(DXL_ID)
    steps -= 1
